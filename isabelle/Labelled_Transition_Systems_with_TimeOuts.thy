...
text \<open>In @{cite rbs}, the term $\mathcal{I}(p) \cap (X \cup \{\tau\}) = \emptyset$ is used a lot, which expresses that there are no immediate transitions the process $p$ can perform (i.e.\@ it is idle) in environments~$X$.\<close>
...
text \<open>\pagebreak
The following corollary is an immediate consequence of this definition.\<close>

...
\isatagdocument
%
\isamarkupchapter{Introduction to Isabelle%
}
\isamarkuptrue%
%
\endisatagdocument
...
%
\begin{isamarkuptext}%
Propositions can be stated using any of the commands \isa{proposition}, \isa{lemma}, \isa{theorem}, \isa{corollary}, and require a proof.
...
\ \ %
\isamarkupcmt{Using the definition of \isa{approx}, we know this about \isa{n} and \isa{m}.%
}\isanewline
...
%
\begin{isamarkuptext}%
The command \isa{function} also establishes equalities, but usually in more complex ways, so that it may not obvious whether a function is well-defined. Hence, the well-definedness needs to be proved explicitly. (These proofs are often solved by the automated proof methods.)

The function is then assumed to be partial. The command \isa{termination} introduces proof obligations to show that the function always terminates (and is thus total). For the example below, this is again proved automatically.

After proving well-definedness and totality, we have access to facts about the function that can be used in proofs, e.g.\@ induction principles. More details can be found in \cref{sec:HML}, where we define a non-trivial \isa{function}.%
\end{isamarkuptext}\isamarkuptrue%
%
\isadelimvisible
%
...
\end{isabellebody}%
\endinput
%:%file=~/projects/Reducing-Reactive-to-Strong-Bisimilarity/isabelle/Isabelle.thy%:%
%:%24=10%:%
%:%36=11%:%
%:%40=13%:%
%:%41=14%:%

...
\example{%
The processes $p$ and $q$ are strongly bisimilar, as both can always perform an a-transition and no transitions afterwards. There is no isomorphism between the left and right subgraphs, as they have a different number of nodes.
...
text \<open>Two processes $p$ and $q$ are then strongly bisimilar (written $p \leftrightarrow q$, following @{cite rbs}) iff there is an SB that contains the pair $(p, q)$.\<close>
